...
over serial. After boot or serial reset, the LCD shows `BOOTING` for up to 8
seconds while waiting for the first bridge payload. If the firmware does not
receive a serial update for 5 minutes, it switches to `disconnected`; the bridge
...
Menu actions send the menu bar app's current quiet setting explicitly so Connect,
Disconnect, and tests stay aligned with the menu state. For CLI/Codex commands,
omit `--quiet-mode` unless you explicitly want to change the board's stored
quiet setting.
...
```text
idle -> working -> pending -> waiting -> error -> disconnected -> usage cue -> usage screen -> idle
```
